<!--writers edit--->
<?php
$sel= $bot->f("SELECT * FROM publisher WHERE id=?",array($G['id']));
$img = !$sel[$i]['img'] ? $G['publisherdefaultimg'] : '/media/'. $sel[$i]['img'];
?>
	<!-- EDIT / SHOW-->
<a href="/publisher">Back to publishers</a>
<span style="float:left;" onclick="g.ui.goto(['previous','writer','id',g.get.id,'/publisher?id='])" class="next glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
<span style="float:right" onclick="g.ui.goto(['next','writer','id',g.get.id,'/publisher?id='])" class="next glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
