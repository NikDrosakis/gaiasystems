    ...
    $.get("https://www.googleapis.com/books/v1/volumes?q="+query,{},function(res){
        ...
        for(var i in res.items){
            items += '<div class="smallcard">' +
                '<div class="author">' + s.implode(',', res.items[i].volumeInfo.authors) + '</div>' +
                '<div class="cover">' +
...
class ActivityManager {
    constructor(maxVisibleActivities = 5, totalActivitiesToShow = 10) {
        this.maxVisibleActivities = maxVisibleActivities;
        this.totalActivitiesToShow = totalActivitiesToShow;
        this.activities = [];
...
//custom ajax upload
const xpost=function(file,params,call,type){
...
const xupload=function(file,formid,call) {
    var url = "/upload.php";
    var xhr = new XMLHttpRequest();
...
//web socket
let ws; // Declare the WebSocket variable globally to be accessible


const soc = (port, indicatorId) => { //G.aconf.config.ws_port
return {
    ws: null,  // Store the WebSocket instance here

    start: (callback) => {
            const user = G.my?.id || 0;
            soc.ws = new WebSocket(`wss://${G.HTTP_HOST}:${port}/${user}`);
            soc.ws.onopen = soc.open;
            soc.ws.onclose = soc.close;
            soc.ws.onmessage = soc.get;

        soc.ws.onerror = (error) => {
                console.error('WebSocket error:', error);
                document.getElementById(indicatorId).className = 'red';
        ...
        switch (data.type) {
            ...
            case 'N':
                for (var key in data.text){
                    $(`.${key}`).html(`<em class="${data.class}">${data.text[key]}</em>`);
                }
                break;
            case 'my':
            case 'io':
...
const WSClient = (port, indicatorId) => {
    return {
        ws: null,  // Store the WebSocket instance here
        pingInterval: null,
        ...
        start: () => {
            return new Promise((resolve, reject) => {
                const user = G.my?.id || 0;
                this.ws = new WebSocket(`wss://${G.HTTP_HOST}:${port}/${user}`);

                // Set up WebSocket event listeners
                this.ws.onopen = (event) => {
                    document.getElementById(indicatorId).className = 'green';
                    this.open(event);
...
//BUFFER + worker
function isWorkersAvailable () {return !!window.Worker;};
const worker=function(params,call,method,responseFormat) {if(isWorkersAvailable()) {
    var wid="w"+hash();
    window[wid] = new Worker("/admin/js/worker4.js");
        ...
        params.responseFormat=!!responseFormat ? responseFormat:'html',
        params.wid=wid;
        params.isWorkerRequest=true;
    window[wid].postMessage(params);
    window[wid].onmessage = call;
}};

//that was the v1.0 to deprecate for a more updated one
function bufAsync(wrap,file,pms,arg,argpms,responseFormat='json'){ //update to chunks //update to worker //+append -prepend
//var cache="bf"+G.my.id+f+G.uname+G.mode+G.page;
    var wrap=String(wrap),
        ...
        params={file:file,pms:pms,arg:arg,page:G.page};
    console.log(params)
//    console.log(argpms)
    return new Promise((resolve,reject) => {
      ...
      //     console.log(res);
            if($('#'+wrap).length==0){
                ...
                $('#'+wrap)[opt](res.data.html);
            }
            //api.red.set(cache,d.data);
            resolve(res.data);},'GET',responseFormat);
        //}
        //})
    })
}
/*
bufAsync upgraded new version 
a) non-blocking 
b) error handling/php error logging 
c) chunking big data 
d) caching using Map, maybe Redis in the future

const bufAsyncInstance = new BufAsyncWorker(); // Will use '/admin/js/worker2.js'
use [wrap]+ to append or [wrap]- to prepend, also use #wrap or .wrap for multiple appends
bufAsyncInstance.bufAsyncWorker('+wrapperId', 'someFile', ['param1', 'param2'], 'methodName', ['arg1', 'arg2'])
    .then(data => {
        console.log("Data loaded and DOM updated:", data);
    })
    .catch(error => {
...
*/
async function loadCubo(wrap,url) {
    try {
...
class AsyncBufferWorker {
    constructor(workerUrl = '/admin/js/worker4.js') {
    ...
    createWorker() {
        if (this.isWorkersAvailable()) {
            const worker = new Worker(this.workerUrl);
            worker.onerror = (e) => {
                const errorMessage = `Worker error: ${e.message} (${e.filename}:${e.lineno})`;
                console.error(errorMessage);
                activityManager.addActivity(errorMessage);
            };
            return worker;
        } else {
            const errorMessage = "Web Workers are not supported in this environment.";
            console.error(errorMessage);
            activityManager.addActivity(errorMessage);
    ...
    async asyncBufferWorker(wrap, file, pms = G) {
        const cacheKey = G.CUR_DIR === 'admin' ? 
            `bf${G.my.id}${f}${G.CUR_DIR}${G.page}${G.sub}` :
            `bf${G.my.id}${f}${G.page}${G.page}${G.id}`;

        const cachedData = this.getCache(cacheKey);
        if (cachedData) {
            this.updateDOM(wrap, cachedData);
            console.log('Operation finished: Data loaded from cache');
            return cachedData;
        }
        const params = { file:file, pms: pms };
      try {
        const response = await this.fetchData(params);
        this.updateDOM(wrap, response.html);
        this.setCache(cacheKey, response);
        console.log('Operation finished: Data fetched and updated DOM');
        return response;
    } catch (error) {
      //  console.warn("Error fetching data:", error.message);
       // activityManager.addActivity(`Error fetching data: ${error.message}`);
        throw error;
    }
    }
    ...
    fetchData(params) {
        return new Promise((resolve, reject) => {
            ...
            let retries = 3; // Number of retries on failure
            ...
            if (retries > 0) {
                ...
                reject(error);
                //activityManager.addActivity(`Fetch data error: ${error.message}`);
                worker.terminate();
            }
        };
          ...
          worker.onmessage = (e) => {
            if (e.data.errors && e.data.errors.length > 0) {
                ...
                e.data.errors.forEach(error => {
                    const errorMsg = `PHP Error: ${error}`;
                    console.error(errorMsg);
             //       activityManager.addActivity(errorMsg);
                });
                handleError(new Error(e.data.errors.join(', ')));
            } else {
                resolve(e.data);
                worker.terminate();
            }
        ...
        worker.onerror = (e) => {
            handleError(new Error(`Worker error: ${e.message}`));
        };
        worker.postMessage(params);
        });
    }
    ...
    updateDOM(wrap, data) {
        const wrapId = String(wrap);
        const opt = wrapId.includes('+') ? 'append' : (wrapId.includes('-') ? 'prepend' : 'html');
        const finalWrap = wrapId.includes('+') || wrapId.includes('-') ? wrapId.slice(0, -1) : wrapId;

        // Check if data is valid HTML
        if (data && !data.error) {
            const elements = $(finalWrap);
            if (elements.length > 0) {
                elements[opt](data);
            } else {
                const warningMessage = `Element with selector ${finalWrap} not found`;
                console.warn(warningMessage);
          //      activityManager.addActivity(warningMessage);
            }
        } else {
            const errorMessage = data?.error || 'Unknown error';
            console.warn(`Invalid HTML or error: ${errorMessage}`);
           // activityManager.addActivity(`Invalid HTML or error: ${errorMessage}`);
        }
    }
...
var s= {
    ...
    loadExt: function (ext, callback) {
        this.loadJS('/admin/js/ext/' + ext + '.js', 'head', callback);
    },
    ...
    chmod: function (file, permission) {
        s.db.get({a: 'chmod', b: file, c: permission}, function (data) {
            // console.log(data)
        })
    },
    shell_exec: function (command, callback) {
        s.db.get({a: 'shell_exec', b: command}, callback);
    },
    cachereset: function () {
        s.db.get({a: 'cachereset'}, function (data) {
            console.log(data);
        });
    },
    sh: function (command) {
        s.db.get({a: 'shell_exec', b: command}, function (d) {
            console.log(d)
        });
    },
    d: (wd) => {
...
//API
const api={
    uri:G.SITE_URL+"api/",version:2.0,port:G.aconf.api_port,
    mo:{url:'mongodb://dros:130177@0.0.0.0:27017/'+G.aconf.config.mongo,
        get:(col,params,call)=>{
            if(typeof(params)!='undefined'){params['a']="get";}else{var params={a:'get'};}
            if(!call){
                $.get(G.aconf.apiuri+'mo/'+col,params,(d)=>{console.log(isjson(d))},'json')
            }else{
                $.get(G.aconf.apiuri+'mo/'+col,params,call,'json');
            }
        },
        count:(col,params,call)=>{
            if(typeof(params)!='undefined'){params['a']="count";}else{var params={a:'count'};}
            if(!call){
                $.get(params,(d)=>{console.log(isjson(d))},'json',G.aconf.apiuri+'mo/'+col)
            }else{
                $.get(G.aconf.apiuri+'mo/'+col,params,call,'json');
            }
        },
        agg:(col,params,call)=>{
            if(!call){
                xget({a:"agg",set:JSON.stringify(params)},(d)=>{console.log(isjson(d))},'json',G.aconf.apiuri+'mo/'+col)
            }else{
                xget(params,call,'json',G.aconf.apiuri+'mo/'+col);
            }
        },
        getOne:(col,params,call)=>{
            if(typeof(params)!='undefined'){params['a']="getOne";}else{var params={a:'getOne'};}
            if(!call){
                xget(params,d=>{console.log(isjson(d))},"json",G.aconf.apiuri+'mo/'+col);
            }else{
                xget(params,call,"json",G.aconf.apiuri+'mo/'+col);
            }
        },
        set:(col,where,params,call)=>{
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"set",where:JSON.stringify(where),params:JSON.stringify(params)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"set",where:JSON.stringify(where),set:JSON.stringify(params)},call,'json');
            }
        },
        up:(col,where,params,call)=>{
            if(!call){xpost(G.aconf.apiuri+'mo/'+col,{a:"upsert",where:JSON.stringify(where),set:JSON.stringify(params)},d=>{console.log(isjson(d))},'json');
            }else{xpost(G.aconf.apiuri+'mo/'+col,{a:"upsert",where:JSON.stringify(where),set:JSON.stringify(params)},call,'json')}
        },
        fup:(col,where,params,call)=>{
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"fup",where:JSON.stringify(where),set:JSON.stringify(params)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"fup",where:JSON.stringify(where),set:JSON.stringify(params)},call,'json');
            }
        },
        upMany:(col,where,params,call)=>{	//updateMany add upsert inc decr
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"upMany",where:JSON.stringify(where),set:JSON.stringify(params)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"upMany",where:JSON.stringify(where),set:JSON.stringify(params)},call,'json');
            }
        },
        del:(col,params,call)=>{
            if(typeof(params)!='undefined'){params['a']="del";}else{var params={a:'del'};}
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,params,d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,params,call,'json');
            }
        },
        ...
        ins:(col,obj,call)=>{
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"ins",set:JSON.stringify(obj)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"ins",set:JSON.stringify(obj)},call,'json');
            }
        },
        insMany:(col,obj,call)=>{
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"insMany",set:JSON.stringify(obj)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"insMany",set:JSON.stringify(obj)},call,'json');
            }
        },
        bulk:(col,obj,call)=>{
            if(!call){
                xpost(G.aconf.apiuri+'mo/'+col,{a:"bulk",set:JSON.stringify(obj)},d=>{console.log(isjson(d))},'json');
            }else{
                xpost(G.aconf.apiuri+'mo/'+col,{a:"bulk",set:JSON.stringify(obj)},call,'json');
            }
        }
    },
    ma:{show:(query,prm,call)=>{
            var prm=!prm ? []:prm;
            if(!call){
                xget({q:query,prm:prm},d=>{console.log(isjson(d))},'json',G.aconf.apiuri+'ma/show');
            }else{
                xget({q:query,prm:prm},call,"json",G.aconf.apiuri+'ma/show');
            }
        },
        get:(query,prm,call)=>{
            var prm=!prm ? []:prm;
            if(!call){xget({q:query,prm:JSON.stringify(prm)},d=>{console.log(d)},"json",G.aconf.apiuri+'ma/get');
            }else{
                xget({q:query,prm:JSON.stringify(prm)},call,"json",G.aconf.apiuri+'ma/get');
            }
        },
        getOne:(query,prm,call)=>{
            var prm=!prm ? []:prm;
            if(!call){xget({q:query,prm:JSON.stringify(prm)},d=>{console.log(d)},'json',G.aconf.apiuri+'ma/getOne');
            }else{
                xget({q:query,prm:JSON.stringify(prm)},call,'json',G.aconf.apiuri+'ma/getOne');
            }
        },
        set:(query,prm,call)=>{
            var prm=!prm ? []:prm;
            if(!call){
                $.post(G.aconf.apiuri+'ma/update',{q:query,prm:prm},d=>{console.log(d)},"json");
            }else{
                $.post(G.aconf.apiuri+'ma/update',{q:query,prm:prm},call,"json");
            }
        },
        del:(query,prm,call)=>{
            var prm=!prm ? []:prm;
            if(!call){
                xpost(G.aconf.apiuri+'ma/delete',{q:query,prm:prm},d=>{console.log(d)},"json");
            }else{
                xpost(G.aconf.apiuri+'ma/delete',{q:query,prm:prm},call,"json");
            }
        },
        ins:(query,array,prm,call)=>{ //??not finished
            var prm=!prm ? []:prm;
            if(!call){
                xpost(G.aconf.apiuri+'ma/insert into',{q:query,prm:prm,ins:JSON.stringify(array)},d=>{console.log(d)},"json");
            }else{
                xpost(G.aconf.apiuri+'ma/insert into',{q:query,prm:prm,ins:JSON.stringify(array)},call,"json");
            }
        },
//if only uid is
        N:(method,myd,list,call)=>{
            ...
            if(typeof myd==="object"){
                api.red.get('N'+myd.uid,old=>{
                    ...
                    if(!call){
                        xpost(G.aconf.apiuri+'ma/N',{q:JSON.stringify(array)},d=>{loadN(d)},"json");
                    }else{
                        xpost(G.aconf.apiuri+'ma/N',{q:JSON.stringify(array)},d=>{call(d);},"json");
                    }
                })
            }else{
        ...
        my:(method,myd,list,call)=>{
            ...
            if(typeof myd==="object"){
                ...
                console.log(array)
                if(!call){
                    xpost(G.aconf.apiuri+'ma/my',{q:JSON.stringify(array)},d=>{
    ...
    red:{
        gfunc:(a,key,call)=>{if(!call){
            xget({key:key},d=>{console.log(d)},'json',G.aconf.apiuri+'red/'+a);
        }else{xget({key:key},call,'json',G.aconf.apiuri+'red/'+a);}
        },
        pfunc:(a,key,val,call)=>{
            var val=typeof(val)=="object" ? JSON.stringify(val):val;
            if(!call){
                xpost(G.aconf.apiuri+'red/'+a,{key:key,val:val},(d)=>{console.log(d)},'json');
            }else{xpost(G.aconf.apiuri+'red/'+a,{key:key,val:val},call,'json');}
        },
        ...
        del:(key,call)=>{
            if(!call){xpost(G.aconf.apiuri+'red/del',{key:key},(d)=>{console.log(d)},'json');
            }else{xpost(G.aconf.apiuri+'red/del',{key:key},call,'json');}},
    },
    form:(name,call)=>{
    ...
    compo:(col,params,call)=>{
        if(!call){xget(G.aconf.apiuri+'compo/'+col,{q:JSON.stringify(params)},(d)=>{
            $('#'+col+'con').html(d);
        })
        }else{xget(G.aconf.apiuri+'compo/'+col,{q:JSON.stringify(params)},call);}
    },
 ...
 * @table :string
 * @clause: string
 * s.db.fl(["tag","id"]),"globs")
 * */
		get:function (params, callback){
		var ajaxfile=G.CUR_DIR=="admin" ? "/admin/index.php" : "/index.php";
			$.get(ajaxfile, params, callback, 'json');
		},
		post:function (params, callback,$type='json') {
		var ajaxfile=G.CUR_DIR=="admin" ? "/admin/index.php" : "/index.php";
			$.post(ajaxfile,params, callback,$type);
		},
        fa:function (query, callback) {
		var ajaxfile = `/admin/index.php?a=fa`;
			$.get(ajaxfile,{query:query},callback,'json');
		},
        fl:function (params, table, callback) {
		var ajaxfile = `/admin/index.php?a=fl&table=${table}`;
			$.get(ajaxfile,{params:params},callback,'json');
		},
        inse:function (table, array, callback) {
		var ajaxfile = "/admin/index.php?a=inse";
        array.table=table;
			$.post(ajaxfile,array,callback);
		},
		/*post:$.post(ajaxfile, {a: 'querychain', b: querya, c: queryb}, callback, 'json');
        querychain: function (querya, queryb, callback) {
            $.post(ajaxfile, {a: 'querychain', b: querya, c: queryb}, callback, 'json');
        },*/
        ...
        query: function (query, callback) {
            var ajaxfile=G.CUR_DIR=="admin" ? "/admin/index.php" : "/index.php";
           console.log(query)
            $.post(ajaxfile, {a: 'func', b: 'q', c: query}, callback);
        },
...
s.file = {
        ...
        download: function(obj){
            s.loadJS("/admin/lib/jquery.fileDownload.js", 'head',function(){
...
//TABLE LOOP
            loop: function (f) {
                var row, nature, divid, event, label, type, query, h, href, datarow, images = 0, board = '', ids = [], mediagrpid;
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        //if img exist
                        // if(images==1) {
                        //     s.ui.table.get_imgs({ids: ids.join(","), mediagrpid: mediagrpid});
                        // }
        ...
        //table produces TABLES- * type:  a | img | button | date * update if img s.db.func, add hidden mediagrp
        tree : function(){
...
s.media= {
    uploader: function (table, mediagrp, linkid,callback) {
		console.log([table, mediagrp, linkid])
        var url = s.ajaxfile,
            uploadButton = $('<button/>')
        ...
        $("#fileupload").fileupload({
            url: url,
            formData: {a: 'insert_obj', mediagrp: mediagrp, linkid: linkid, table: table},
            dataType: 'json',
            autoUpload: false,
	...
	select: function(obj){
		...
		s.db.fa("SELECT * from obj GROUP BY filename",function (imgs) {
			console.log(imgs)
			var modaldata='';
			for(var i in imgs){
...
/*	s.cors("https://vivalibro.gr:6984/admin/_find",JSON.stringify(find),function(res){
	...
	$(document).on("click","button[id^='update_gaia']",function(data){
        ...
        s.db.get({a:"gaia_update",b:version},function(res){
			console.log(res);
	...
	$(document).on("click","button[id^='template_update']",function(data){
        ...
        s.db.get({a:"template_update",b:version},function(res){
			console.log(res);
			if(res[0]=="copied" && res[1]=="extracted"){
			s.ui.notify("success","GaiaCMS Update successful")
			}else if (res=='nossh'){
...
s.ui.form = {
    get: function (ob, callback) {
        //   console.log(ob)
        callback = !callback ? s.callback : callback;
        var append = 'append' in ob ? ob.append : (s.get.dsh ? '.gs-sidepanel' : '#main_window');
        inp = '';
        ...
        if (!ob.nature) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var z in fo) {
                        ...
                        f.alias = 'alias' in fo[z] ? fo[z].alias : s.ucfirst(f.row);
                        f.placeholder = 'placeholder' in fo[z] ? fo[z].placeholder : s.ucfirst(f.row);
                        f.type = fo[z].type;
                        f.global = fo[z].global;
                        f.format = 'format' in fo[z] ? fo[z].format : '';
                        f.mediagrpid = 'mediagrpid' in fo[z] ? fo[z].mediagrpid : '';
                        f.table = ob[s].adata;
            ...
            for (var i in ob.list) {
                ...
                ob.placeholder = 'placeholder' in ob.list[i] ? ob.list[i].placeholder : s.ucfirst(ob.row);
    ...
    newsubmit: function (obj,callback) {
        event.preventDefault();
        var form = $("#" + obj.id).serializeArray();
        console.log(form);
        $.post("/admin/index.php",form,callback)
    },
...
//TYPE DROP DOWN
        if (f.type == 'img') {
            if (f.format == 'form-upload') {
                s.ui.form.upload.file.get(f.mediagrpid, f.get, data.img, f.table);
            }
            result = '';
        } else if (f.type == 'drop') {
            var key;
...
//TYPE TEXT | NUMBER | DATE
            ...
            string = f.type == 'date' ? date('Y-m-d', data[f.row]) : data[f.row];
...
//TYPE HIDDEN
    ...
    textarea: function (obj, textarea, formid) {
        ...
        var id = explode('_', obj.id)[2];
        var inpid = obj.id.replace('submit_', '');
        // var value = s.htmlentities.encode(textarea.value);
        var q = f.query.replace("@row", id).replace("@value", s.htmlentities.encode(value));
        // console.log(value)
        s.db.func('query', q, function (data, textStatus, jqXHR) {
            if (data == 'yes') {
                s.alert('Page Saved');
            } else {
                console.log(data);
    ...
    drop: function (obj) {
        // console.log(this.id)
        var id = s.explode('_', obj.id)[1];
        // var value=$("#"+f.inputid).val();
        var q = f.query.replace("@row", id).replace("@value", obj.value);
        // console.log(q)
        s.db.func('query', q, function (data) {
            if (data == 'yes') {
                s.alert("Value updated!");
            } else {
                s.alert("Value updated!");
            }
        })
    ...
    upload: {
        file: {
            ...
            get: function (mediagrpid, id, img) {
                s.loadJS(s.path.lib+'jquery.form.min.js');
                var board = '<div class="imgBox">' +
                    ' <div id="imgView"><img src="' + (img ?  img : "/admin/img/post.jpg") + '" style="height:250px;width:250px;"></div>' +
                    ...
                    '<form action="' + s.ajaxfile + '" onsubmit="s.ui.form.upload.file.submit(this,event)" id="upload" method="post" enctype="multipart/form-data">' +
                    ...
                    '<input type="hidden" name="mediagrpid" value="' + mediagrpid + '">' +
        ...
        img: {
            folder: s.siteurl + 'gaia/lib/uploader/',
            append: ".post_container",
            filename: '',
            get: function (mediagrp, linkid, file, table) {
                $(this.append).prepend("<script src='/admin/lib/jquery.ui.widget.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/load-image.all.min.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/canvas-to-blob.min.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload-process.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload-image.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload-audio.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload-video.js'></script>");
                $(this.append).prepend("<script src='/admin/lib/jquery.fileupload-validate.js'></script>");
                s.loadCSS(this.folder + "css/jquery.fileupload.css");
                s.loadJS(this.folder + "css/style.css");
                ...
                if (typeof file != 'undefined' && file != null) {
                    $('#files').html('<img src="' + s.get.UPLOADS + file + '" style="height:250px;margin: -21px 0 0 -21px;">');
                } else {
                    // $.get(g.ajaxfile,{a:'select_obj',b:mediagrp,c:linkid,d:table},function(data){
                    //     console.log(mediagrp);
                    // if(data!='no') {
...
s.ui.list = {
    ...
    head : function(){
        ...
        list_head = '<button id="' + table+'_reset" style="float:left" onclick="s.ui.list.reset(this)" class="btn btn-default btn-sm">Reset</button>'+
            '<input type="text" id="' + table+'_search" style="width: 80%;display:inline-flex;float: left;margin-left: 1%;" onkeyup=" s.ui.list.search(this)" placeholder="search" value="'+(coo(table+"'_search") ? coo(table+"_search") :"")+'" class="form-control input-sm">'+
            // <!----line 2 DATE FROM ---->
            '<div class="toFromTitle">'+
            '<span>Date from:</span><input style="display:inline-flex" type="date" onchange="s.ui.list.dateselection(this)" value="'+(coo(table+"_from") ? coo(table+"_from"):"")+'" id="' + table+'_from" class="form-control input-sm">'+
            '</div>'+
            // <!----line 3 DATE FROM ---->
            '<div class="toFromTitle">'+
            '<span>to:</span><input style="display:inline" type="date" class="form-control input-sm" onchange="s.ui.list.dateselection(this)" value="'+(coo(table+"_to") ? coo(table+"_to"):"")+'" id="' + table+'_to">'+
            '</div>'+
        ...
        for(i in orders) {
            list_head += '<option value="' + orders[i] + '" ' + (coo(table+"_order") && orders[i] == coo(table+"_order").split(" ")[2] ? "selected=selected" : "") + '>' + orders[i] + '</option>';
        }
        list_head += '</select>'+
            // <!----ORDER POST SORT---->
        ...
        for(i in this.order){
            list_head += '<option value="'+this.order[i]+'" '+(coo(table+"_order") && this.order[i]==coo(table+"_order").split(" ")[3] ? "selected=selected":"")+'>'+this.order[i]+'</option>';
        }
        list_head += '</select>'+
        ...
        for(i in taxuri) {
            list_head += '<option value="'+taxuri[i]+'" '+(coo(table+"_select")==taxuri[i] ? "selected=selected":"")+'>'+taxuri[i]+'</option>';
        }
        list_head += '</select>'+
            '</div>'+
            // <!----line 6 PAGINATION ---->
    ...
    get: function () {
        ...
        var select= !coo(table+'_select') ? this.param.select : coo(table+'_select');
        var search= !coo(table+'_search') ? this.param.search : coo(table+'_search');
        var from= !coo(table+'_from') ? this.param.from : coo(table+'_from');
        var to= !coo(table+'_to') ? this.param.to: coo(table+'_to');

        //set page 1
        if(!coo(table+'_page')){coo(table + '_page', 1);}
        var page= coo(table+'_page');
...
s.chart = {
    pie : function (div, file,vol) {
        ...
        d3.json(g.ajaxfile+'?a=chart00', function(error,data) {
            // d3.csv("/admin/dsh/data.csv", function(error, data) {
            // console.log(data)
...
s.editor= {
    ...
    sethead: '<!DOCTYPE HTML>\n' +
        ...
        '<!-- ====Stylesheets===== -->\n' +
        '<script src="/admin/js/libs.js"></script>\n' +
...
s.upload= {
    uploader: function (table, mediagrp, linkid,callback) {
        ...
        var formdata= {a: 'insert_obj', mediagrp: mediagrp, linkid: linkid, table: table};
...
function makeResizableRightSidebar(elementId) {
    const sidebar = document.getElementById(elementId);
    const container = sidebar.parentElement;
    let isResizing = false;
    sidebar.addEventListener('mousedown', function(e) {
        // Start resizing if the mouse is near the left edge of the sidebar
        if (e.offsetX < 10) {
            isResizing = true;
    ...
    function resizeSidebar(e) {
        ...
        if (newWidth > parseInt(window.getComputedStyle(sidebar).minWidth) &&
            newWidth < parseInt(window.getComputedStyle(sidebar).maxWidth)) {
            sidebar.style.width = `${newWidth}px`;
            coo('sidebar_width',newWidth);
        }
    }
