<?php $menulist= $this->db->fl(array('id','title'),'links'); ?>
<div id="mainpage">
    <button id="newmenuBtn" onclick="s.ui.switcher('#newmenuBox','','inline-block')">New menu</button>
...
<!---INSERT AND CREATE menu--->
 <div id="newmenuBox" style="display:none">
    ...
    $query= $this->db->q("INSERT INTO links (title,children) VALUES(?,?)",array(
        $title,
        $children
    ));
    if($query)redirect(0);
...
<div id="mBox<?=$menu?>">
<button onclick="s.ui.switcher(['#mealias_<?=$menu?>','#malias_<?=$menu?>'],'','inline-block')"  class="pencil glyphicon glyphicon-pencil"></button>
<button class="btn btn-xs btn-danger" style="float:right" id="delmenu<?=$menu?>">X</button>
<label>
   <span id="malias_<?=$menu?>"><?=$menualias?></span>
   <input id="mealias_<?=$menu?>" style="display:none" value="<?=$menualias?>">   
</label>

   <button id="newlinkbut<?=$menu?>" onclick="s.ui.switcher('#newlinkbox<?=$menu?>')" class="btn btn-success btn-xs">+</button>
...
MENU LINKS
------------------------------------>
<?php $sel= $this->db->fa("SELECT * FROM links WHERE linksgrpid=? ORDER BY sort",array($menu));  ?>
   ...
   <li style="cursor:move" id="nodorder<?=$menu?>_<?=$selid?>" class="list-group-item menuBox<?=$menu?>">
      <button class="pencil glyphicon glyphicon-pencil" id="close<?=$selid?>" onclick="s.ui.switcher(['#read_<?=$menu?><?=$selid?>','#edit_<?=$menu?><?=$selid?>'],'','inline-block')"></button>
...
<script src="/admin/main/menu/menu.js"></script>