/*updated:2020-01-29 20:20:34 menu - v.0.73 - Author:Nikos Drosakis - License: GPL License*/
		...
		//make sortable
		s.ui.sortable("UPDATE links SET sort=? WHERE id=?","li");
//edit menulink
		$(document).on("keyup","input[id^='title"+G.langprefix+"_'],input[id^='uri_']",function(){
			var exp= s.explode('_',this.id);
			var val= this.value;
			console.log("UPDATE links SET "+exp[0]+"='"+val+"' WHERE id="+exp[2]);
			s.db.query("UPDATE links SET "+exp[0]+"='"+val+"' WHERE id="+exp[2],function(data){
				if(data!='No'){
...
//edit menutitle
		$(document)
			.on("keyup","input[id^='mealias_']",function(){
				...
				s.db.query("UPDATE menu SET title"+G.langprefix+"='"+val+"' WHERE id="+exp[1],function(data){
					if(data!='No'){
			...
			*************************/
			...
			.on("click","button[id^='savenewmenu']",function() {
				...
				var query= "INSERT INTO links (menuid,title"+G.langprefix+",uri,sort) VALUES("+menuid+",'"+title+"','"+uri+"',0)";
//				console.log(query)
				s.db.query(query,function(data){
					location.reload();
				});
			})
			...
			// 	s.db.func("query",query,function(){location.reload();});
			// })
			//del uri
			.on("click","button[id^='delmenu']",function() {
				var id= this.id.replace('delmenu','');
				s.db.query("DELETE FROM menu WHERE id="+id,function(){$('#mBox'+id).hide();});
			})
			.on("click","button[id^='delink']",function() {
				var menu= s.explode('_',this.id)[1];
				var id= s.explode('_',this.id)[2];
								s.db.query("DELETE FROM links WHERE id="+id,function(){$('.menuBox'+id).remove();});
