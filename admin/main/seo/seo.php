...
<?php foreach ($this->xmls as $file){
    ...
    <div class="gs-databox" style="display:block">
        <div class="gs-title" onclick="s.ui.switcher('#content<?=$file?>','slide')"><?=$filename?>
        ...
        <!-----editor------>
        <pre><code contenteditable="true" class="language-php line-numbers gs-databox-inside" id="content<?=$file?>"><?=file_exists(PUBLIC_ROOT_WEB.$filename) ? htmlentities(file_get_contents(PUBLIC_ROOT_WEB.$filename)):''?></code></pre>
    </div>
<?php } ?>
...
<script src="/admin/main/seo/seo.js"></script>