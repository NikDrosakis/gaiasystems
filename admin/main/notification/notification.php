<style>
    ...
    <div class="form-section">
        <label for="uid">User ID (for "one" cast)</label>
        <input id="uid" type="text" placeholder="Enter User ID" disabled>
    </div>
    ...
    <div class="form-section">
        ...
        <input id="fun" type="text" placeholder="Enter Function">
    </div>
...
<script>
    ...
    function updatePreview() {
        ...
        if (cast === 'one') previewText += `User ID: ${uid}<br>`;
        if (rule) previewText += `Rule: ${rule}<br>`;
        if (fun) previewText += `Function: ${fun}<br>`;
        previewText += `Text: ${text}<br>`;
    ...
    document.getElementById('cast').addEventListener('change', function() {
        ...
        uidInput.disabled = this.value !== 'one';
        updatePreview();
    });

    document.getElementById('type').addEventListener('change', updatePreview);
    document.getElementById('rule').addEventListener('input', updatePreview);
    document.getElementById('fun').addEventListener('input', updatePreview);
    ...
    document.getElementById('execute').addEventListener('click', () => {
        ...
        if (data.cast === 'one' && !document.getElementById('uid').disabled) {
            data.uid = document.getElementById('uid').value;
        }
        console.log(data);
        soc.ws.send(JSON.stringify(data));
    });
