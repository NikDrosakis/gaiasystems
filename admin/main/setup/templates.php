...
function add2couch(template,ver,sum){	
	...
	console.log(newdata)
	//admin:n130177!@
	 s.cors("https://api.nikosdrosakis.gr:6984/templates/"+_id,JSON.stringify(newdata),function(res){ console.log(res);},'PUT');
}
/*
...
*/
		...
		s.cors('https://api.nikosdrosakis.gr:6984/templates/_all_docs?include_docs=true', {}, function (data) {
			...
			//console.log(dat)
			//var templates = s.array_diff(data.rows, G.templates),
			var box = '';
			var sum=0;
...
*/
		$(document).on("click",".preview",function(data){
            ...
            s.dialog({title: title ,success: {},callback: function () {},message: "<iframe id='iframe"+name+"' style='width:100%;float:left;position:relative;height:88vh;' frameBorder='0'  src='"+link+"' sandbox='allow-same-origin allow-scripts allow-popups allow-forms'></iframe>"})
		})
			...
			//	s.shell_exec(command,function(res){
				...
				$.get(s.ajaxfile,{a:"template_install",b:template},function(res){
					...
					if(res[0]=="copied" && res[1]=="extracted"){
						...
						$('#' + templatename).remove();		
						//del setup entry
					//		s.db.query("INSERT INTO setup (name,created) VALUES('" + template + "','" + s.time() + "')");
					}else if (res=='nossh'){
...
// 					}
// 				}
// 				// s.file.file_put_contents(nheader, s.ui.sethead + '<title>' + array[k][i] + '</title>\n' + s.file.htmldecode(array[k][i]) + '\n</head>\n<body>\n');
// 				// s.file.file_put_contents(nbody1, array[i][2] + array[i][3])
// 				// s.file.file_put_contents(nfooter, array[i][4])
...
//            })
            ...
            .on("click", "button[id^='uninstall']", function () {
                ...
                s.confirm("Warning! Are you sure you want to delete the template <b>" + template + "</b>? This action will be irreversible.", function (res) {
                    if (res == true) {
					 	$.ajax({
					 		type: "GET",
					 	//	dataType: "json",
							...
							//if (data == 'yes') {
                                ...
                                $('#box' + template).remove();
                                s.ui.notify('success', 'Template ' + template + ' Uninstalled', 'successfully.')
                            //}
            	...
            	var template = this.id.replace('setup', '');
                        ...
                        $.get(s.ajaxfile, {a: 'template_setup', b: template}, function (data) {
                            ...
                            if (data == 'yes') {
                                ...
                                s.ui.notify('success', 'Template ' + template + ' Setup', 'successfully.')
                            }
                        })
            })
			*/
            ...
            .on("click", "button[id^='binstall']", function () {
                ...
                s.confirm("Your template will be changed. Are you sure?", function (res) {
                    if (res) {				
