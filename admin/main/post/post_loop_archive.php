...
$postid=$sel[$i]['id'];
		$img=$sel[$i]['img']=='' ? '/admin/img/myface.jpg': UPLOADS.$sel[$i]['img'];
...
<span><a href="/admin/user?uid=<?=$sel[$i]['uid']?>"><?=$sel[$i]['username']?></a></span>
<h3 style="margin-top: 2%;margin-bottom: 2%;">
<a style="color:#000000" href="/<?=$sel[$i]['uri']?>"><?=$sel[$i]['title'.$langprefix]?></a>
<button onclick="location.href='/admin/post?id=<?=$postid?>'" style="position:absolute" type="button" class="close" aria-label="delete" id="del<?=$postid?>"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>
</h3>
<div class="excerpt">
...
<div class="tag">
<a href="/tax?id=1"><?=$sel[$i]['taxname']?></a>
</div>
<?php } ?>
