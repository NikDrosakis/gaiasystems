...
$postid=$G['id'];
//selected or parent
$selected= $this->db->fl("taxid","post","WHERE id=$postid");
//xecho($grps);
//tags
...
?>
<div id="box_taxonomy">
    <a href="/admin/tax" class="gs-title">Taxonomy</a>
    <?php if(!empty($grps)){for($j=0;$j<count($grps);$j++){
        ...
        ?>
        <br/>
        <label style="color:red"><a href="/admin/tax?id=<?=$grpid?>"><?=$grps[$j]['name']?></a></label>
...
<!--                    <option value="--><?//=$stax[$i]['id']?><!--" --><?//=(!empty($selected) && in_array($stax[$i]['id'],$selected)) || in_array($stax[$i]['id'],$this->tax()->parents_ofselected_children($G['id'])) ? "selected=selected":""?><!-->--><?//=$stax[$i]['name']?><!--</option>-->
