...
if($a=='exec'){
    ...
    $select=$this->db->f("SELECT en FROM globs WHERE name=? AND tag=?",[$b,'widget']);
    ...
    $select=$this->db->f("SELECT en FROM globs WHERE name=? AND tag=?",[$b,'widget']);
    if(!empty($select)){
        $query=$this->db->q("UPDATE globs SET en=? WHERE name=? and tag=?",[$c,$b,'widget']);
    }else{
        $query=$this->db->inse('globs',['name'=>$b,'en'=>$c,'tag'=>'widget']);
    }
    echo !$query ? 'NO':'OK';
...
if($a=='chmod') {
    ...
    echo json_encode($this->db->columns($b));

}elseif($a=='sorting'){
    foreach ($_POST['order'] as $order => $id){
        xecho($_POST['query']);
        $this->db->q($_POST['query'],array($order,$id));
    }
...
//                $filename=$tag !=0 ? greeklish($name).'_'.$tag.'.'.$ext : greeklish($filename).'.'.$ext;
            ...
            $count=$this->count_("id","media"," WHERE filename=?",array($filename));
...
//                }
            ...
            $ins=$this->db->q("INSERT INTO media (uid,mediagrpid,filename,created) VALUES(?,?,?,?)",
            ...
            $ins2= $this->db->q($query,array(UPLOADS.$filename,$_POST['id']));
            ?>
    ...
    $sel['loop']= $this->db->fa("$query $fromto $searchQ $selectQ $groupby $order $limit");

    if(!empty($sel)){
        $sel['q'] = "$query $fromto $searchQ $selectQ $order $limit";
        $sel['count'] = $count = count($this->db->fa("$query $fromto $searchQ $selectQ $groupby"));
        echo json_encode($sel);
    }else{
        echo json_encode($sel);
    }
/*
    ...
    if ($search!=""){
        foreach($this->db->char_types($table) as $col){
            $s[]= "$table.$col LIKE '%$search%'";
        }
    ...
    $sel= $this->db->$a("$b $fromto $searchQ $order $limit");

    if(!empty($sel)){
        $sel[0]['query'] = "$b $fromto $searchQ $order $limit";
        $sel[0]['count'] = $count = count($this->db->$a("$b $fromto $searchQ"));
        echo json_encode($sel);
    }else{
        echo json_encode("No");
    }
...
*/
    ...
    $querya= $this->db->q($b,array(),'id');
    $queryb= $this->db->q($c,array($querya));
    if($queryb){
    ...
    //$sel = $this->db->q("UPDATE $table SET $value WHERE $where");
    $sel = $this->db->q($c);
    if ($sel) {
...
echo json_encode($this->db->fa($_GET['query']));

}elseif($a=='fl'){
echo json_encode($this->db->fl($_GET['params'],$_GET['table']));
    ...
    $sel= $this->db->fetchList(array('mediaview.linkid','media.filename'),"media",
    ...
    $insert=$this->db->inse($table,$_REQUEST);
    ...
    unset($_POST['table']);
    //    if($table !=""){
    //        $id=$this->db->fetchMax('id',$table)+1;
    //$pnames=array();
    //$pvalues=array();
    //        $pnames[]='id';
    //        $pvalues[]=$id;
    ...
    //xecho($_POST);
    $insert=$this->db->inse($table,$_POST);
    ...
    //get max id+1
    //$new_pvar= (int)$this->db->fetchMax("id","globs")+1;
    //insert id
    $query= $this->db->q("INSERT INTO globs(name,tag) VALUES(?,?)",array($b,'pvar'));
    if (!$query){ echo "No";}
    ...
    $sel= $this->db->fa("SELECT * FROM globs $impl");
    //xecho($impl);
    if(!empty($sel)) {
        echo json_encode($sel);
    }
...
* */
    ...
    echo json_encode($this->db->f("SELECT * FROM user WHERE id=?",array($b)));
...
//$this->q("UPDATE global ")
    ...
    //  $query= $this->db->q("DELETE FROM setup WHERE name=?",array(trim($b)));
    // if($query) {
    if(xrmdir($this->TEMPLATESURI . $b)){
        echo 'yes';
    }else{echo $b;}
//}elseif( $a=='template_setup'){
    //  $query= $this->db->q("INSERT INTO setup (name) VALUES(?)",array(trim($b)));
//    if($query) {
    //echo 'yes';
    //}
...
//$fileName = $_SERVER['DOCUMENT_ROOT'].'/page1.php';
    ...
    $s= $this->db->fl(array("uid","name"),"user2","WHERE uid IN (1,300)");
