...
module.exports = function(params, config) {
    ...
    const call = async function() {
        ...
        try {
            ...
            if (action === 'get') {
                sql = 'SELECT * FROM cubo_tasks; SELECT * from links;';
                ...
                // Insert into cubo_tasks
                sql = `INSERT INTO cubo_tasks (system_id, cubos_id, text, start_date, end_date, progress, parent, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
                ...
                const { id } = params;
                sql = 'UPDATE cubo_tasks SET text = ?, start_date = ?, end_date = ?, progress = ?, duration = ?  WHERE id = ?';
                pms = [text, start_date, end_date, progress, duration, id];
                await conn.query(sql, pms);
                return { success: true };
            } else if (action === 'delete') {
                const { id } = params;
                sql = 'DELETE FROM cubo_tasks WHERE id = ?';
                pms = [id];
                await conn.query(sql, pms);
                return { success: true };
            } else {
